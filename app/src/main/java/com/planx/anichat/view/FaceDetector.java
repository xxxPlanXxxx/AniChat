...
public class FaceDetector {
    ...
    public FaceDetector(Context c, CallActivity activity) {
    ...
    public void getLandmarks(Bitmap bmp) {
        ...
        synchronized (this) {
            ...
            final String sEmotion = MyApplication.emotion[0]+"@"+MyApplication.emotion[1]+"@"+MyApplication.emotion[2]+"@"+MyApplication.emotion[3];
                    new Thread(new Runnable() {
                        @Override
                        public void run() {
                            Log.i("messageInstantSend",sEmotion+MyApplication.callWho);
                            MyApplication.the().getmAgoraAPI().messageInstantSend(MyApplication.callWho,0,sEmotion,"");
                        }
                    }).start();
...
//        if(yaw < Math.PI / 18) {
        ...
        MyApplication.emotion[0] = yaw;
        MyApplication.emotion[1] = pitch;
        MyApplication.emotion[2] = roll;
    ...
    private void solveEmotion(ArrayList<Point> landmarks, int width) {
        //mouth open
        MyApplication.emotion[3] = (double)(landmarks.get(57).y-landmarks.get(51).y) /
